...
export function LiveTracker() {
  ...
  if (!booking) {
    return <Card className="p-12 text-center text-muted-foreground rounded-[2rem] border-border/50 bg-card/60 backdrop-blur-xl shadow-xl">No bookings to track.</Card>;
  }
      ...
      <Card className="rounded-[1.5rem] border-border/50 bg-card/60 p-6 sm:p-8 backdrop-blur-xl shadow-lg relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <div className="text-3xl font-black text-foreground">Booking <span className="text-primary">#{booking.id}</span></div>
            <div className="mt-2 text-sm font-semibold text-muted-foreground flex flex-wrap items-center gap-2">
              <span className="bg-background/50 px-2 py-1 rounded-md border border-border/50 shadow-sm">{booking.vehicleName}</span>
              <span className="opacity-50">/</span>
              <span className="font-mono text-primary bg-primary/10 px-2 py-1 rounded-md">{booking.vehiclePlate}</span>
              <span className="opacity-50">/</span>
              <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" /> {booking.scheduledAt}</span>
            </div>
          </div>
              ...
              <SelectContent className="rounded-xl border-border/50 bg-card/90 backdrop-blur-xl">
                {bookings.map((item) => (
                  <SelectItem key={item.id} value={item.id} className="font-medium">
                    #{item.id} <span className="opacity-50 mx-1">/</span> <span className="font-mono text-primary">{item.vehiclePlate}</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
        ...
        {isCancelled ? (
          <div className="relative z-10 flex flex-col items-center gap-4 py-8 text-center animate-in zoom-in-95 duration-500">
            ...
            <div>
              ...
              <div className="text-sm font-medium text-muted-foreground mt-1">This booking has been cancelled and cannot be tracked.</div>
            </div>
            <span className={cn("mt-4 rounded-full border px-4 py-1 text-xs font-bold uppercase tracking-wider shadow-sm", STATUS_STYLES.Cancelled)}>
              Cancelled
            </span>
          </div>
        ) : (
          <div className="relative z-10">
            ...
            <div className="relative hidden items-start justify-between md:flex">
              ...
              {STEPS.map((step, index) => {
                ...
                return (
                  <div key={step.key} className="relative z-10 flex flex-1 flex-col items-center text-center group">
                    <div
                      className={cn(
                    ...
                    >
                      {done ? <Check className="h-7 w-7" /> : <Icon className={cn("h-7 w-7", active ? "animate-pulse" : "")} />}
                    </div>
                    <div className={cn("mt-5 font-bold text-base transition-colors", active ? "text-primary" : done ? "text-foreground" : "text-muted-foreground/70")}>{step.label}</div>
                    <div className="mt-1 text-xs font-medium text-muted-foreground">{step.desc}</div>
                    {active && (
                      <span className={cn("mt-3 rounded-full border px-3 py-0.5 text-[10px] font-bold uppercase tracking-widest shadow-sm absolute -bottom-8 animate-in slide-in-from-top-2", STATUS_STYLES[step.key])}>
                        Current
                      </span>
                    )}
                  </div>
            ...
            <div className="space-y-6 md:hidden relative">
              ...
              {STEPS.map((step, index) => {
                ...
                const Icon = step.Icon;
                return (
                  <div key={step.key} className={cn("flex gap-5 relative z-10", active ? "scale-105 origin-left transition-transform" : "")}>
                    <div
                      className={cn(
                    ...
                    >
                      {done ? <Check className="h-5 w-5" /> : <Icon className={cn("h-5 w-5", active ? "animate-pulse" : "")} />}
                    </div>
                    <div className="flex flex-col justify-center">
                      <div className={cn("font-bold text-base", active ? "text-primary" : done ? "text-foreground" : "text-muted-foreground/70")}>{step.label}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="rounded-[1.5rem] border-border/50 bg-card/60 p-8 backdrop-blur-xl shadow-lg transition-all hover:shadow-xl">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
            ...
            <InfoItem label="Check-in Time" value={booking.checkInAt ? new Date(booking.checkInAt).toLocaleString() : "Waiting"} />
            ...
            <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-border/50 mt-2">
              <InfoItem label="Status" value={booking.status} badge={STATUS_STYLES[booking.status]} />
              ...
              <InfoItem label="Loyalty" value={`+${booking.checkoutPointsEarned ?? 0} / -${booking.checkoutPointsRedeemed ?? 0}`} />
            </div>
          </dl>
        </Card>
        ...
        <Card className="rounded-[1.5rem] border-border/50 bg-card/60 p-8 backdrop-blur-xl shadow-lg transition-all hover:shadow-xl">
          ...
          {transaction ? (
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
              ...
              <InfoItem label="Final Amount" value={fmtBookingMoney(transaction.finalAmount)} highlight className="sm:col-span-2 p-4 bg-background/50 rounded-xl border border-border/50" />
              <div className="sm:col-span-2 p-4 bg-emerald-500/10 rounded-xl border border-emerald-200/50 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-800">Points Earned</span>
                <span className="font-bold text-emerald-600 bg-background px-2 py-1 rounded-md shadow-sm">+{transaction.pointsEarned} pts</span>
              </div>
            </dl>
          ) : (
            <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground/70">
              ...
              <div className="text-sm font-medium">
                Checkout summary will appear here<br/>after payment is completed.
              </div>
            </div>
          )}
        </Card>
...
function InfoItem({ label, value, highlight, badge, className }: { label: string; value: string; highlight?: boolean; badge?: string; className?: string }) {
  return (
    <div className={className}>
      <dt className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1.5">{label}</dt>
      <dd>
        {badge ? (
          <span className={cn("rounded-md border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider shadow-sm inline-flex", badge)}>
            {value}
          </span>
        ) : (
          <span className={cn("font-medium", highlight ? "text-lg font-black text-primary" : "text-foreground")}>{value}</span>
        )}
      </dd>
    </div>
  );
